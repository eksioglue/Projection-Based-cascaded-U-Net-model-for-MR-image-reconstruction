...
from dclayer.updated_dclayer import updated_DClayer_esc as updated_DClayer
import timeit
...
class Unet_Model(Model):
    ...
    def updatedcascad(self, input, mean, std, masked_kspace, inv_mask):
        ...
        outUDC1 = updated_DClayer(unet1, masked_kspace, inv_mask, mean, std)

        unet2 = self.forward(outUDC1)
        outUDC2 = updated_DClayer(unet2, masked_kspace, inv_mask, mean, std)

        unet3 = self.forward(outUDC2)
        outUDC3 = updated_DClayer(unet3, masked_kspace, inv_mask, mean, std)

        unet4 = self.forward(outUDC3)
        outUDC4 = updated_DClayer(unet4, masked_kspace, inv_mask, mean, std)

        unet5 = self.forward(outUDC4)
        outUDC5 = updated_DClayer(unet5, masked_kspace, inv_mask, mean, std)
        return outUDC5
